    ...
    try {
        ...
        if ($action === 'submit_attendance') {
            ...
            try {
                ...
                $stmt = $pdo->prepare("SELECT firstname, lastname FROM pafe_acc WHERE id = ?");
                $stmt->execute([$_SESSION['user_id']]);
                $user_profile = $stmt->fetch(PDO::FETCH_ASSOC);
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO pafe_attendance (fullname, status, event_id, created_at) 
                    VALUES (?, 'Pending', ?, NOW())
                ");
                $stmt->execute([$fullname, $event_id]);
    ...
    <header class="header" id="header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center">
                ...
                // Fallback values
                $full_name = $user_profile['firstname'] . ' ' . $user_profile['lastname'] ?? $_SESSION['user_name'] ?? 'John Doe';
                $email = $user_profile['email'] ?? $_SESSION['email'] ?? '';
                $profile_picture = $user_profile['img'] ?? 'default.png';
                ...
                <div class="modal-body">
                    <form id="manageProfileForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="firstname" name="firstname" value="<?php echo htmlspecialchars($full_name); ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="lastname" class="form-label">Last Name</label>
                            <input type="text" class="form-control" id="lastname" name="lastname" value="<?php echo htmlspecialchars($full_name); ?>" required>
                        </div>
                        <div class="mb-3">
                                ...
                                <form method="POST">
                                    ...
                                    if ($user_profile): ?>
                                        <div class="alert alert-info">
                                            <p><strong>Name:</strong> <?php echo htmlspecialchars($user_profile['firstname'] . ' ' . $user_profile['lastname']); ?></p>
                                        </div>
