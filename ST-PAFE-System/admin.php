...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    if (empty($firstname) || empty($lastname) || empty($email) || empty($user_id)) {
        header("Location: ?page=home&error=First name, last name and email are required.");
        exit;
    }
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM pafe_acc WHERE email = ? AND id != ?");
    ...
    $img = $_SESSION['profile_picture'] ?? 'default.png';
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            if (in_array($file['type'], $allowed_types) && $file['size'] <= $max_size) {
                ...
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    ...
                    if ($img !== 'default.png' && file_exists($upload_dir . $img)) {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO pafe_acc (id, firstname, lastname, email, img)
        VALUES (?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
            ...
            img = VALUES(img)
    ");
    $stmt->execute([$user_id, $firstname, $lastname, $email, $img]);
...
try {
    if (isset($_SESSION['user_id'])) {
        $stmt = $pdo->prepare("SELECT firstname, lastname, email, img FROM pafe_acc WHERE id = ?");
        $stmt->execute([$_SESSION['user_id']]);
        $user_profile = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($user_profile) {
            $_SESSION['user_name'] = $user_profile['firstname'] . ' ' . $user_profile['lastname'];
            $_SESSION['email'] = $user_profile['email'];
            $_SESSION['profile_picture'] = $user_profile['img'] ?? 'default.png';
        }
    }
} catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_report') {
    ...
    try {
        if ($report_type === 'events') {
            ...
            fputcsv($output, ['Full Name', 'Event', 'Status', 'Created At']);
            ...
            $stmt = $pdo->query("SELECT f.*, e.event_name FROM pafe_feedback f LEFT JOIN pafe_events e ON f.event_id = e.id");
            $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($data as $row) {
                fputcsv($output, [
                    $row['event_name'] ?? '',
        ...
        if ($action === 'create_event' || $action === 'update_event') {
            ...
            if ($action === 'create_event') {
                ...
                if ($stmt->fetchColumn() > 0) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO pafe_events (event_name, event_date, event_time, status, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
        ...
        if ($action === 'create_attendance' || $action === 'update_attendance') {
            ...
            if ($action === 'create_attendance') {
                ...
                if ($stmt->fetchColumn() > 0) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO pafe_attendance (fullname, gender, year_level, section, status, event_id, date, time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute([$fullname, $gender, $year_level, $section, $status, $event_id, $date, $time]);
                ...
                if ($stmt->fetchColumn() > 0) {
                    ...
                    $stmt = $pdo->prepare("UPDATE pafe_attendance SET fullname = ?, gender = ?, year_level = ?, section = ?, status = ?, event_id = ?, date = ?, time = ? WHERE id = ?");
                    $stmt->execute([$fullname, $gender, $year_level, $section, $status, $event_id, $date, $time, $id]);
...
try {
    $stmt = $pdo->query("SELECT a.*, e.event_name, e.event_date, e.event_time 
...
<head>
    ...
    <style>
        ...
        .content {
            ...
            background: #fff !important; /* Debug background */
            color: #000 !important; /* Debug text color */
        }
        .content.expanded {
            margin-left: 80px;
        }
